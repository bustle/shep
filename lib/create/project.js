...
exports.default = function () {
  ...
  }]).then(createProjectFolder).then(createApi).then(createDirectories).then(createFiles);
  ...
  function createFiles(params) {
    return _bluebird2.default.all([_fsExtraPromise2.default.writeJSONAsync(params.folder + '/api.json', { name: params.name, region: params.region, id: params.id, functionNamespace: params.namespace, accountId: params.accountId }), _fsExtraPromise2.default.writeJSONAsync(params.folder + '/package.json', { name: params.name, devDependencies: { 'babel-preset-es2015': "^6.3.13" } }), _fsExtraPromise2.default.writeFileAsync(params.folder + '/env.js.example', "module.exports = { beta: { env: 'beta', secret: 'beta-key' }, production: { env: 'production', secret: 'prod-key'} }"), _fsExtraPromise2.default.writeFileAsync(params.folder + '/.gitignore', 'node_modules/*\nenv.js'), _fsExtraPromise2.default.writeFileAsync(params.folder + '/README.md', '#' + params.name), _fsExtraPromise2.default.writeJSONAsync(params.folder + '/.babelrc', { "presets": ["es2015"] }), _fsExtraPromise2.default.writeJSONAsync(params.folder + '/.jshintrc', {
      "node": true,
      "esnext": true,
      "eqeqeq": true,
      "indent": 2,
